...
function Table(name, schema, options, base) {
    ...
    if (this.options.continuous_backup === undefined || this.options.continuous_backup === null) {
        this.options.continuous_backup = false;
    }
}
...
Table.prototype.createIndex = function createIndex(attributes, indexSpec) {
    ...
    base.ddb().updateTable(params, function (err, data) {
        if (err) {
            params.BillingMode = 'PAY_PER_REQUEST';
            params.GlobalSecondaryIndexUpdates = params.GlobalSecondaryIndexUpdates.map(function (index) {
                if (index.Create)
                    delete index.Create.ProvisionedThroughput;
                else if (index.Update)
                    delete index.Update.ProvisionedThroughput;
                return index;
            });
...
Table.prototype.setupContinuousBackup = function () {
    if (this.options.continuous_backup) {
        var deferred = Q.defer();
        var params = {
            PointInTimeRecoverySpecification: {
                PointInTimeRecoveryEnabled: this.options.continuous_backup
            },
            TableName: this.name
        };
        this.base.ddb().updateContinuousBackups(params, function (err, data) {
            ...
            .then(function (data) {
                ...
                if (table.options.update) {
                    ...
                    Q()
                        ...
                        .then(function () {
                            return table.setupContinuousBackup();
                        })
                        .catch(function (e) {
                            debug(e);
                        });
