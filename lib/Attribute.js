...
function Attribute(schema, name, value) {
    ...
    this.schema = schema;
    this.name = name;
    this.encrypt = schema.options.encrypt !== undefined && schema.options.encrypt[name] === true;

    if (this.encrypt) {
        this.options.toDynamo = (data) => {
            if (!data)
                return null;
            return {S: data};
        }
        this.options.fromDynamo = (data) => {
            return data.S || this.parseDynamo(data, true);
        }
    }
    ...
    if (!schema.useDocumentTypes) {
        this.type = convertToDocumentTypes(this.type);
    }

    if (schema.useNativeBooleans) {
        this.type = convertToNativeBooleans(this.type);
    }

    this.attributes = setupAttributes(this.type, value, schema, name);
...
function convertToDocumentTypes(type) {
    if (type.name === 'map') {
        debug('Overwriting attribute %s type to object', this.name);
        return this.types.object;
    } else if (type.name === 'list') {
        debug('Overwriting attribute %s type to array', this.name);
        return this.types.array;
    }
    return type;
}

function convertToNativeBooleans(type) {
    if (type.name === 'boolean') {
        debug('Overwriting attribute %s type to be a native boolean', this.name);
        return this.types.nativeBoolean;
    }
    return type;
}

function setupAttributes(type, value, schema, name) {
    ...
    if (type.name === 'map') {
        ...
        for (var subattrName in value) {
            if (attributes[subattrName]) {
                throw new errors.SchemaError('Duplicate attribute: ' + subattrName + ' in ' + name);
            }

            attributes[subattrName] = module.exports.create(schema, subattrName, value[subattrName]);
        }
        ...
        if (value === undefined && value[0] === undefined) {
            throw new errors.SchemaError('No object given for attribute:' + name);
        }

        if (value.length > 1) {
            throw new errors.SchemaError('Only one object can be defined as a list type in ' + name);
        }
        ...
        for (var i = 0; i < value.length; i++) {
            attributes[i] = module.exports.create(schema, 0, value[i]);
        }
    }
...
Attribute.prototype.getType = function (value) {
    if (!value) {
    ...
    if (util.isArray(typeVal) && typeVal.length === 1 && (typeof typeVal[0] === 'object' || typeof typeVal[0] === 'function')) {
        type = 'List';
...
Attribute.prototype.toDynamo = function (val, noSet, model) {
    ...
    if (this.encrypt !== true && this.validator && !this.validator(val, model)) {
...
Attribute.prototype.parseDynamo = function (json, skipCustom = false) {

    if (!skipCustom && this.parseDynamoCustom) {
